...
To evaluate whether your card will work:

✔ 24 distinct color patches
✔ Regular 6 × 4 grid layout
✔ Uniform spacing (rows and columns align)
✔ Includes:
    -neutral gray/black/white patches
    -saturated colors (reds, yellows, blues, greens)
...
project_folder/
├─ scripts/            ← Python scripts and .bat launchers
├─ images_raw/         ← Original images with color checker
├─ images_corrected/
├─ color_data/
└─ analysis_results/
...
-Polygon mode: Multiple pumpkins per image, use if you have multiple pumpkins in an image. Allows you to outline each pumpkin and save color data for each as a separate row.
